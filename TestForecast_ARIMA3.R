Test.Forecast3<-function(serie,Nobs=288,Track=TRUE,Anderson = FALSE, Parsimony = TRUE, BoxJenkins = TRUE,Horizon=12,InformationCriterion = "AIC",wndow=96, Transform=TRUE){
  if(length(serie)>=((Nobs-1)+wndow)){
    ...
    return(list(Measures=Measures,track = track,fit=fit,forecast=Estimation, lower=L_DM,upper=U_DM, Time=c(mean(alg_time, na.rm = TRUE),max(alg_time, na.rm = TRUE)),Parameters_Num=c(mean(Parameters_no, na.rm = TRUE),median(Parameters_no, na.rm = TRUE),max(Parameters_no, na.rm = TRUE))))
